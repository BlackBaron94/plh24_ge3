...
public class Controller {
    ...
    public static class WikiControllerImpl implements WikiController {
        ...
        private final packageAPI.WikiApiClient apiClient;
...
public WikiControllerImpl() {
    this(new packageAPI.WikiApiClient());
}

public WikiControllerImpl(packageAPI.WikiApiClient apiClient) {
...
public List<SearchResultRow> search(String keywords, SearchMode mode, List<String> checkedCategories) {
    if (mode == SearchMode.DB_API) {
        try {
            List<packageAPI.WikiApiClient.ApiSearchHit> hits = apiClient.searchHits(keywords);

            // Στο search call δεν έχουμε categories -> βάζουμε κενή λίστα
            return hits.stream()
                    .map(h -> new SearchResultRow(h.title, h.pageId, "API", List.of()))
                    .toList();

        } catch (Exception ex) {
            // fallback σε dummy αν πέσει το API
            return filterByCategories(dummySearchPool, checkedCategories);
        }
    }

    // DB_ONLY (προς το παρόν dummy)
      return filterByCategories(dummySearchPool, checkedCategories);
}   
